...
var selectTextDisplay = "Max Select [NUMBER] Numbers"; //select number text display
...
var revealTimer = 5; //reveal ball timer

var prizeTableDisplay = "Score Table"; //score table text display
var numberTextDisplay = ""; //your score text display
var matchTextDisplay = "MATCH [NUMBER]"; //match text display
var scoreTextDisplay = "[NUMBER]X"; //prize score text display
...
var resultCompleteText = "YOU WON [NUMBER] SOL"; //result complete text display
...
var gameData = { paused: true, sphereX: 800, sphereY: 325, cageRadius: 195, radius: 0, spin: false, selectNum: 0, numberNum: 0, numberArray: [], selectArray: [], winArray: [], buttonArray: [], indexArray: [], dimArray: [], matchNum: 0, ballsArray: [], cageArray: [], revealArray: [], totalBall: [], ballNumber: [] };
...
function buildGameButton() {
                ...
                .then(x => {
                    if (x) {
                        phantomWallet.getBalance().then(x => {
                            ...
                            walletIdtxt.text = myStr.substr(0, 4) + '******' + myStr.substr(myStr.length - 4, myStr.length);
                            txtCredit.text = textCredit.replace('[NUMBER]', (x / solanaWeb3.LAMPORTS_PER_SOL).toFixed(10));
    ...
    betconfirm.cursor = "pointer";
    betconfirm.addEventListener("click", function(evt) {
...
async function createTrans() {
    ...
    await phantomWallet.requestTransaction(Number(gameData.selectArray.length * 0.05)).then(result => {
        {
            transbet = {
                ...
                "typeOfPlay": "SOL",
                "betAmount": gameData.selectArray.length * 0.05,
                "selectArray": (gameData.selectArray.map(item => item + 1)).toString(),
                "totalCards": gameData.selectArray.length,
            };
            Notify("Transaction Successful");
            document.getElementById("gif").style.display = "none";
            buttonSphereStart.visible = true;
            betconfirm.visible = false;
            canSelect = false;
        }
    }).catch((err) => {
    ...
    switch (page) {
        ...
        case 'result':
            ...
            if (gameData.matchNum != -1) {
                ...
                resultTitleTxt.text = resultCompleteText.replace('[NUMBER]', addCommas(playerData.score > 0 ? win : 0));
...
function setSelectBalls() {
    ...
    for (var n = 0; n < gameData.selectArray.length; n++) {
        ...
        newGuessBall.scaleX = newGuessBall.scaleY = .9;
        newGuessBall.x = startX;
        newGuessBall.y = startY;
...
function endGame() {
    ...
    paid = 0;
    score = 0;
    const intersection = gameData.selectArray.filter(element => gameData.revealArray.includes(element));
    intersection.length > 0 ? score = intersection.length : score = 0;
    ...
    if (score > 0) {
        win = score_arr[score - 1].prize * 0.05 * intersection.length;
        paid = win - win * 0.015;
    };
    postData = {
            ...transbet,
            "revealArray": (gameData.revealArray.map(item => item + 1)).toString(),
            "amountWon": intersection.length > 0 ? win : 0,
            "gameResult": intersection.length > 0 ? "WIN" : "LOSS",
            "amountPaid": intersection.length > 0 ? paid : 0,
            "multiplier": intersection.length > 0 ? score_arr[score - 1].prize : 0,
            "amountLost": intersection.length > 0 ? 0 : transbet['betAmount'],
        }
        // console.log(postData);
    axios.post(`${DB_URL}/api/instantLotto`, {
        ...postData
    });
...
function createPercentage() {
    ...
    var extraBall = bonusBall == true ? 1 : 0;
    var totalNum = gameData.selectArray.length;
    var totalBall = 0;
    var percent = 0;
